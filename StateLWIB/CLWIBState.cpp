...
void CLWIBState::PrepPreviews() {
   ...
   blocks.push_back(PreviewBlock = CApplication::get().getSceneManager().addMeshSceneObject(cubeMesh, DiffuseTexture, mat));
...
void CLWIBState::PrepBlock(float x, float y, int w, int h) {
   ...
   blocks.push_back(tempBlock = CApplication::get().getSceneManager().addMeshSceneObject(cubeMesh, DiffuseTexture));
   placeables.push_back(tempPlaceable = new CBlock(x, y, w, h));
   CMaterial mat;
   mat.Texture = CImageLoader::loadTexture("Textures/dirt.bmp");
   tempBlock->setMaterial(mat);
