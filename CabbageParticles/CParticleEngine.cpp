...
CParticleEngine::CParticleEngine(SVector3 pos, int max, float duration, int particleType) {
   ...
   */
   std::vector<SVector3*> positionArrTemp;
   std::vector<SVector3*> colorArrTemp;
   std::vector<float> sizeArrTemp;

   for(int i = 0; i < max; i++) {
      positionArrTemp.push_back(new SVector3((float)rand()/(float)RAND_MAX*1 - 0.5, (float)rand()/(float)RAND_MAX*2, 0));
      colorArrTemp.push_back(new SVector3((float)rand()/(float)RAND_MAX*0.8 + 0.2, (float)rand()/(float)RAND_MAX*0.8 + 0.2, (float)rand()/(float)RAND_MAX*0.8 + 0.2));
      sizeArrTemp.push_back((float)rand()/(float)RAND_MAX*30 + 1);
   }
   /*
   positionArr = positionArrTemp;
   sizeArr = sizeArrTemp;
   colorArr = colorArrTemp;
   */
