...
void CElementEnemyBanana::setupPhysicsEngineObject() {
   ...
   PhysicsEngineObject->getActorAttributes().MaxWalk = 0.8f;
...
void CElementEnemyBanana::setupSceneObject() {
   ...
   if (Level.getEnvironment() == Env::FOREST) {
      ...
      mesh = CMeshLoader::load3dsMesh("Base/desert_banana.3ds");
   }
   else if (Level.getEnvironment() == Env::WATER)
      mesh = CMeshLoader::load3dsMesh("Base/water_banana.3ds");
